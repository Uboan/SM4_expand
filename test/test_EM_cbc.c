 ...
 * @LastEditors: zyp
 * @LastEditTime: 2022-11-02 02:41:57
 * @FilePath: /sm4_256/test/test_EM_cbc.c
 * @Description: 
 * 
...
int main(){
      ...
      uint8_t plaintext[NUMBER] = {0x89, 0x15, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
    ...
    //   sm4_256_EM_cbc_set_key(key2,ks);
    //   sm4_256_EM_cbc_encrypt(input,output,ivec,NUMBER,key1,ks);
    //   printf("\n密文：");
    //   dump_hex(output,NUMBER);
    //   sm4_256_EM_cbc_decrypt(output,output1,ivec,NUMBER,key1,ks);
    //   printf("\n解密完的明文：");
	...
	for(amount=16;amount<=1048576;amount*=4){
		...
		while(time(NULL)<endwait)
		{
			...
			sm4_256_EM_cbc_decrypt(input,output,ivec,amount,key1,ks);
		}
