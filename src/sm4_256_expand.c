...
int sm4_expand_set_key(const uint8_t *key, SM4_EXPAND_KEY *ks)
{
     ...
     * Family Key
     */
    static const uint32_t FK[8] =
        { 0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc, 0xa27011dc, 0xc562013d, 0xa2345c1d, 0xa98074ac6 };
    ...
    K[4] = load_u32_be(key, 4) ^ FK[4];
    K[5] = load_u32_be(key, 5) ^ FK[5];
    K[6] = load_u32_be(key, 6) ^ FK[6];
    K[7] = load_u32_be(key, 7) ^ FK[7];

    for (i = 0; i != SM4_KEY_SCHEDULE; ++i) {
        uint32_t X = K[(i + 1) % 8] ^ K[(i + 2) % 8] ^ K[(i + 3) % 8] ^ CK[i];
        uint32_t t = 0;
...
void sm4_expand_encrypt(const uint8_t *in, uint8_t *out, const SM4_EXPAND_KEY *ks)
{
    ...
    SM4_RNDS(32, 33, 34, 35, SM4_T);
    SM4_RNDS(36, 37, 38, 39, SM4_T);
    SM4_RNDS(40, 41, 42, 43, SM4_T_slow);
...
void sm4_expand_decrypt(const uint8_t *in, uint8_t *out, const SM4_EXPAND_KEY *ks)
{
    ...
    SM4_RNDS(43, 42, 41, 40, SM4_T_slow);
    SM4_RNDS(39, 38, 37, 36, SM4_T);
    SM4_RNDS(35, 34, 33, 32, SM4_T);
