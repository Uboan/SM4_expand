 ...
 * @LastEditTime: 2022-11-02 07:46:05
 * @FilePath: /sm4_256/src/lut_sm4_even_mansour.c
 */
//------------------sm4.c---------------------
...
/**
 * @brief SM4 cipher
 * @param in 128bit input
 * @param out 128bit output
 * @param sm4_key SM4 roundKey
 * @param enc enc(0)or dec(1)
 */
static void _SM4_do(uint8_t* in, uint8_t* out,uint8_t *key1, SM4_Key sm4_key, uint8_t enc);
...
void SM4_LUT_EM_Encrypt(uint8_t* plaintext, uint8_t* ciphertext,uint8_t* key1, SM4_Key sm4_key) {
    _SM4_do(plaintext, ciphertext, key1, sm4_key,0);
}

void SM4_LUT_EM_Decrypt(uint8_t* ciphertext, uint8_t* plaintext,uint8_t* key1, SM4_Key sm4_key) {

    _SM4_do(ciphertext, plaintext, key1, sm4_key,1);
}

void SM4_LUT_EM_KeyDelete(SM4_Key sm4_key) { free(sm4_key); }
#define XOR_KEY( )       \
      do {               \
         x[0] ^= KEY_x[0];     \
         x[1] ^= KEY_x[1];     \
...
static void _SM4_do(uint8_t* in, uint8_t* out,uint8_t *key1, SM4_Key sm4_key, uint8_t enc) {
    ...
    //load key1
    if(enc==0){
    KEY_x[0]=load_u32_be(key1,0);
    KEY_x[1]=load_u32_be(key1,1);
    KEY_x[2]=load_u32_be(key1,2);
    KEY_x[3]=load_u32_be(key1,3);

        
    }else{
    KEY_x[0]=load_u32_be(key1,3);
    KEY_x[1]=load_u32_be(key1,2);
    KEY_x[2]=load_u32_be(key1,1);
    KEY_x[3]=load_u32_be(key1,0);
